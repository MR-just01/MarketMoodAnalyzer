...
def score_sentiment(news_df):
    if news_df.is_empty(): 
        return pl.DataFrame(schema={"Date": pl.Date, "FinBERT_Score": pl.Float64})
    ...
    # 👇 ADD TQDM HERE 👇
    print("🔍 Analyzing headlines with AI...")
    headlines = news_df.get_column('Headline')
    
    for headline in tqdm(headlines, desc="Scoring News (FinBERT)", unit="article"):
